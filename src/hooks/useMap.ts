...
import { dbMapToMap, type DbMap, type Map, type FogRegion, type DrawingRegion, type EffectTile } from '../types';
...
export const useMap = () => {
  ...
  const updateEffectData = useCallback(
    async (mapId: string, effectData: EffectTile[]): Promise<{ success: boolean; error?: string }> => {
      try {
        const { error } = await supabase
          .from('maps')
